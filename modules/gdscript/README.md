...
Scripts can be at different stages of compilation. The process isn't entirely linear, but consists of this general order: tokenizing, parsing, analyzing, and finally compiling. Source scripts use this complete process. Binary-token and compiled-module resources retain enough front-end data to fall back safely when their cached result cannot be accepted.
...
On supported 64-bit desktop and Android targets, the experimental `gdscript_baseline_jit` build option also compiles eligible typed functions to native code when their bytecode is finalized. Participating `bool`, `int`, `float`, `Vector2`, `Vector3`, `Color`, large engine value types, and trivial user-defined structs stay unboxed in aligned native frames for the duration of execution. Functions whose complete argument and return signature uses supported native layouts also receive a pointer-based typed entry, allowing normal script method dispatch to skip construction of the callee's `Variant` stack after exact type validation. Validated native static calls and native calls on `self` are lowered to `MethodBind::ptrcall()` when all arguments and the optional result have compatible native layouts. Other receivers, non-trivial signatures, calls requiring conversion, debugger or call-stack tracking, resumed coroutines, and unsupported bytecode retain the Variant-frame interpreter-compatible path.

Eligible functions are promoted after repeated calls to a compact optimizing tier. It constructs block SSA with loop and merge phis, performs constant and branch folding, local value numbering, copy elimination, and dead pure-node elimination, then emits a new native entry while keeping the baseline entry as its fallback. `Vector2`, `Vector3`, and `Color` values are scalar-replaced into independently live components; math fields read from trivial user structs enter the same representation, while struct construction, updates, equality, typed returns, and control-flow phis preserve the layout's native size and alignment. The debugger profiler writes portable hot-function keys and bytecode fingerprints to `.godot/gdscript_optimization_profile`; release exports include this profile so exact matches are promoted eagerly on the target. Machine code is never serialized, and stale fingerprints are ignored after script changes.


### Portable compiled modules

[`GDScriptCompiledModule`](gdscript_compiled_module.h) defines the versioned `.gdm` container used by the editor cache and binary-script exports. It stores integer VM bytecode, constants, names, frame metadata, portable class metadata, and symbolic descriptions of evaluator, constructor, utility, `MethodBind`, and lambda tables. Class records cover inheritance, names, members and their types, static members, constants, signals, RPC configuration, nested classes, lambda captures, and initializer/function bindings. Function records also preserve argument and return types, method information, RPC configuration, and named local constants. Script and struct types are represented by stable paths, qualified names, and schema descriptors rather than addresses. Native addresses and target-specific JIT code are never written to the file. The header includes source, dependency, bytecode-format, and engine/Variant API fingerprints plus a payload checksum. The original binary-token stream is embedded as the initial fallback.

The current format deliberately keeps parsing and analysis as a verification oracle. After the normal front end creates the class and lambda graph, every deserialized class record must exactly match its freshly compiled symbolic metadata. Each portable function must likewise match its signature, expected bytecode fingerprint, constants, frame shape, default-argument targets, and complete relocation table before any records are installed. All symbols must resolve against the current engine API. A corrupt or stale module is discarded atomically and execution continues with bytecode compiled from its embedded tokens. This is more conservative than a standalone bytecode loader, but establishes pointer-free class and function schemas before they become responsible for constructing runtime objects directly.

Editor loads probe `.godot/gdscript_compiled/` using the source and API fingerprints, and successful compilations refresh that cache. Binary-token export modes precompile project scripts to `.gdm` and remap eligible `.gd` resources to those modules. Unsupported functions use the embedded token fallback, while scripts that fail module creation retain the legacy `.gdc` export. Export also writes `.godot/gdscript_project.gdmanifest`, a sorted whole-project dependency/function index. Exact optimization-profile matches are recorded per function so the target can compile the compact SSA tier eagerly; mismatches retain normal hotness-based promotion.
