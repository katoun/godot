...
[`GDScriptCompiledModule`](gdscript_compiled_module.h) defines the versioned `.gdm` container used by the editor cache and binary-script exports. It stores integer VM bytecode, constants, names, frame metadata, and symbolic descriptions of evaluator, constructor, utility, `MethodBind`, and lambda tables. Native addresses and target-specific JIT code are never written to the file. The header includes source, dependency, bytecode-format, and engine/Variant API fingerprints plus a payload checksum. The original binary-token stream is embedded as the initial fallback.

The first format revision deliberately keeps parsing and analysis as a verification oracle. After the normal front end creates the class and lambda graph, each portable function must match its expected bytecode fingerprint, constants, frame shape, default-argument targets, and complete relocation table before any records are installed. All symbols must resolve against the current engine API. A corrupt or stale module is discarded atomically and execution continues with bytecode compiled from its embedded tokens. This is more conservative than a standalone bytecode loader, but establishes a pointer-free file and relocation ABI without weakening validation.
