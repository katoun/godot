...
Development metadata lives in a separate length-delimited section with its own version. It records instruction-address-to-line/column mappings, lexical local-variable lifetimes, profiler identifiers, and editor member locations. Direct-loaded functions therefore retain stack traces, breakpoint line opcodes, local inspection, profiling identity, and hot-reload behavior. Editor caches and debug exports include this section; release exports omit it while keeping identical executable metadata and fingerprints. Cache and direct-load failures retain a phase-specific fallback reason so editor diagnostics can explain why the front end was used.

The `.gdm` file starts with a small, independently versioned compatibility envelope. The mandatory `.gdc` fallback and the portable payload have separate sizes and checksums, so token recovery does not require parsing the payload format. Format or bytecode version changes, engine API changes, stale dependencies, unknown metadata, failed relocations, invalid bytecode, unsupported feature flags, and corrupt portable payloads all reject direct loading before runtime state is committed and compile the recovered tokens instead. Only a corrupt or unsupported envelope, a damaged fallback checksum, or a missing mandatory fallback prevents this recovery. The envelope has an explicit fallback-presence flag so embedding tokens can become an export policy after direct loading is mature; current module creation and binary exports continue to require them.
